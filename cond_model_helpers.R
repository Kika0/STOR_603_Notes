  ...
  for (j in given) {
    ...
    for (i in 2:d) {
      ...
      if (margin=="GenGaus" & method!="one_step") {
        opt <- optim(fn=NLL_GenGaus,x=(Y2-a_hat[length(a_hat)]*Y1)/(Y1^b_hat[length(b_hat)]),par=c(0,1,1.5),control=list(maxit=2000),method = "SANN")
        mu_agg_hat <- append(mu_agg_hat,opt$par[1])
        sig_agg_hat <- append(sig_agg_hat,opt$par[2])
      ...
      if (margin=="AGG" & method!="one_step") {
        opt <- optim(fn=NLL_AGG,x=(Y2-a_hat[length(a_hat)]*Y1)/(Y1^b_hat[length(b_hat)]),par=c(0,1,1.2,1.8),control=list(maxit=2000),method = "SANN")
        mu_agg_hat <- append(mu_agg_hat,opt$par[1])
        sig_agg_hat <- append(sig_agg_hat,opt$par[2])
        deltal_hat <- append(deltal_hat,opt$par[3])
        deltau_hat <- append(deltau_hat,opt$par[4])
      ...
      if (margin=="AGGsig" & method!="one_step") {
        opt <- optim(fn=NLL_AGGsig,x=(Y2-a_hat[length(a_hat)]*Y1)/(Y1^b_hat[length(b_hat)]),par=c(0,1,1,1.5),control=list(maxit=2000),method = "SANN")
        mu_agg_hat <- append(mu_agg_hat,opt$par[1])
      ...
      if (margin=="AGGsigdelta" & method!="one_step") {
        opt <- optim(fn=NLL_AGGsigdelta,x=(Y2-a_hat[length(a_hat)]*Y1)/(Y1^b_hat[length(b_hat)]),par=c(0,1,1,1.2,1.8),control=list(maxit=2000),method = "SANN")
        mu_agg_hat <- append(mu_agg_hat,opt$par[1])
        sigl_hat <- append(sigl_hat,opt$par[2])
        sigu_hat <- append(sigu_hat,opt$par[3])
...
map_param <- function(tmp_est,method = "AGG", facet_var = "cond_site",title_map="") {
  ...
  pmu <- tm_shape(uk_tmp1) + tm_dots(col="mu",style="cont",size=0.3,palette="-RdBu",colorNA=misscol,midpoint=0,title=TeX("$\\mu$"), breaks=lims) + tm_facets(by=facet_var,nrow = nrow_facet) +  tm_layout(panel.labels = facet_label,legend.outside.size=legend_outside_size,asp=0.5,legend.text.size = 1,legend.title.size=1.5, title=title_map) 
  ...
  psig <- tm_shape(uk_tmp1 %>% filter(is.na(sig) |sig < sigmax)) + tm_dots(col="sig",style="cont",size=0.3,palette="viridis",colorNA=misscol,title=TeX("$\\sigma$"), breaks=lims) + tm_facets(by=facet_var,nrow = nrow_facet) +  tm_layout(panel.labels = facet_label,legend.outside.size=legend_outside_size,asp=0.5,legend.text.size = 1,legend.title.size=1.5, title=title_map) 
    ...
    pmuagg <- tm_shape(uk_tmp1) + tm_dots(col="mu_agg",style="cont",size=0.3,palette="-RdBu",colorNA=misscol,midpoint=0,title=TeX("$\\mu_{AGG}$"), breaks=lims) + tm_facets(by=facet_var,nrow = nrow_facet) +  tm_layout(panel.labels = facet_label,legend.outside.size=legend_outside_size,asp=0.5,legend.text.size = 1,legend.title.size=1.5, title=title_map) 
