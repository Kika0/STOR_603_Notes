...
tmp <- c()
for (i in 1:81) {
  tmp[((i-1)*360+1):(i*360)] <- c((uk_winter[is_location=="london",((90*(i-1))+7):((90*i)+6)] %>% as_vector),
                                      (uk_spring[is_location=="london",((90*(i-1))+7):((90*i)+6)] %>% as_vector),
                                      (uk_summer[is_location=="london",((90*(i-1))+7):((90*i)+6)] %>% as_vector),
                                      (uk_autumn[is_location=="london",((90*(i-1))+7):((90*i)+6)] %>% as_vector))
}
tmp <- cbind(uk_winter[7:ncol(uk_winter)],uk_spring[7:ncol(uk_winter)],uk_summer[7:ncol(uk_winter)],uk_autumn[7:ncol(uk_winter)]) %>% filter(is_location=="london") %>% as_vector()
...
ggplot() + 
  ...
  geom_point(aes(x=-1/log(df1$q),y=df1$dat)) + coord_trans(x="log") +
...
ggplot() + 
  geom_line(aes(x=-1/log(df$p),y=df$z)) +
  geom_point(aes(x=-1/log(df1$q),y=df1$dat)) + coord_trans(x="log") + xlab("Return level (years)") +
  ylab(TeX("$z_p$"))
