...
fb <-  fit_boot %>% arrange(mu) %>% slice(3:98)
top_bottom <- fb %>% slice(c(1,n()))
# probboot <- pgev(q=x,loc=fit$mle[1],scale=fit$mle[2],shape=fit$mle[3])
# boot <- boot[probboot> 0.025 & probboot <0.975]
# probboot <- probboot[probboot> 0.025 & probboot <0.975]
# bootstrap <- data.frame(boot,probboot)
zu <- GEV_inv(U=f,mle=as.numeric(top_bottom[2,]))
zl <- GEV_inv(U=f,mle=as.numeric(top_bottom[1,]))
