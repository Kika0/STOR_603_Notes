...
# val <- c(X_1,X_2,Y)
# ite <- c(1:N,1:N,1:(N*d))
# var <- c(rep("X_1",N),rep("X_2",N),rep("Y",(N*d)))
# df <- data.frame(val,var,ite)
# ggplot(df) + geom_point(aes(x=ite, y=val,color=var),alpha=0.5)

# ggplot(df %>% filter(var != "Y")) + geom_point(aes(x=ite, y=val,color=var),alpha=0.5)

# it is better to plot X_1/N against X_2/N
plot(X_1,X_2)
df1 <- data.frame(X_1=X_1,X_2=X_2,uni=seq(0.02,1,0.02),
             uni_3=  seq(0.02,2/3,length.out=50), 
             u_x=seq(0,0.1,length.out=50),
             u_y=0.1-seq(0,0.1,length.out=50),
             line_1=seq(0.02,1,0.02)*(1/2),line_2=seq(0.02,1,0.02)*(1),line_3=seq(0.02,2/3,length.out=50)*(3/2))
df1 <-df1 %>%  mutate(above_u=(X_1+X_2>0.1)) %>% mutate(N=rep(nrow(df1),nrow(df1)))
...
df2 <- data.frame(X_1=X_1,X_2=X_2,uni=seq(0.02,1,length.out=N),
             uni_3=  seq(0.02,2/3,length.out=N), 
             u_x=seq(0,0.1,length.out=N),
             u_y=0.1-seq(0,0.1,length.out=N),
             line_1=seq(0.02,1,length.out=N)*(1/2),line_2=seq(0.02,1,length.out=N)*(1),line_3=seq(0.02,2/3,length.out=N)*(3/2))
df2 <-df2 %>%  mutate(above_u=(X_1+X_2>0.1)) %>% mutate(N=rep(nrow(df2),nrow(df2)))

ggplot(df2) + geom_point(aes(x=X_1, y=X_2,col=above_u),alpha=0.5) +
  scale_color_manual(values = c("TRUE" = "black",
                                "FALSE"="#C11432")) +
   labs(color=TeX('Above threshold $u$')) +
  xlab(TeX("$X_1/n$")) + ylab(TeX("$X_2/n$"))+
  geom_line(aes(x=uni,y=line_1),alpha=0.1) +
    geom_line(aes(x=uni,y=line_2),alpha=0.1) +
    geom_line(aes(x=uni_3,y=line_3),alpha=0.1) +
   geom_line(aes(x=u_x,y=u_y),col="#C11432",alpha=0.6) +
...
```{r}
df3 <- data.frame(r=(X_1+X_2)*N,w=X_1/(X_1+X_2),above_u=df2$above_u)
ggplot(df3) + geom_point(aes(x=w,y=log(r),col=above_u))+
scale_color_manual(values = c("TRUE" = "black",
                                "FALSE"="#C11432")) +
   labs(color=TeX('Above threshold $u$')) 
# add second plot with higher sample size
