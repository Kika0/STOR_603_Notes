...
```{r}
sAIC <- tmpdfs %>% filter(pair_index=="NO_NO2") %>% pull(AIC)
tmpdf1 <- tmpdf %>% filter(pair_index=="NO2_NO") %>% mutate(AICsum=AIC+sAIC)
...
```{r}
tmpdf1[1:5,]
tmpdfs %>% filter(pair_index=="NO_NO2", family %in% tmpdf1[1:5,7])
```
...
fit_copula_two_dataset <- function(data1=obsresw,data2=obsress) {
  ...
  rvinecopulib::vinecop(data=obsresw, selcrit = "mbicv") %>% summary()
