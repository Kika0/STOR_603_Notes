...
tmpdfw <- tmpdfs <-  data.frame(pair_index=as.numeric(),u1=as.numeric(),u2=as.numeric(),tau=as.numeric(),AIC=as.numeric(),familyname= as.character(),family=as.numeric(),par=as.numeric(),par2=as.numeric())
# write a loop for all pairs in the first tree
for (m in 1:(d-1)) {
  index_var1 <- tree_struc[1,m]
  index_var2 <- tree_struc[(d-m+1),m]
  # calculate all possible copulas
  for (i in 1:length(index_cop)) {
    x <- BiCopEst(u1=as.numeric(obsresw[,index_var1]),u2=as.numeric(obsresw[,index_var2]),family=index_cop[i])
    tmpdfw <- rbind(tmpdfw,data.frame(pair_index=paste0(names(winter)[index_var1+1],"_",names(winter)[index_var2+1]),u1=index_var1,u2=index_var2,tau=x$tau,AIC=x$AIC,familyname=x$familyname,family=x$family,par=x$par,par2=x$par2))
   y <- BiCopEst(u1=as.numeric(obsress[,index_var1]),u2=as.numeric(obsress[,index_var2]),family=index_cop[i])
    tmpdfs <- rbind(tmpdfs,data.frame(pair_index=paste0(names(winter)[index_var1+1],"_",names(winter)[index_var2+1]),u1=index_var1,u2=index_var2,tau=y$tau,AIC=y$AIC,familyname=y$familyname,family=y$family,par=y$par,par2=y$par2))
  

# fix copula families in the first tree

  }
  tmpdf <- tmpdfw %>% mutate(taus=tmpdfs$tau) %>% rowid_to_column()
tmpdf1 <- tmpdfw %>% mutate(tausum=abs(taus+tau))
tmpdf1 <- tmpdf1 %>% group_by(pair_index) %>%  arrange(desc(tausum)) %>% relocate(tausum,taus,.after=tau) %>% slice(1)

  tmpdf5w <- tmpdfw %>% mutate(tausum=abs(taus+tauw))
tmpdf5w <- tmpdf5w %>% group_by(pair_index) %>%  arrange(desc(tausum)) %>% relocate(tausum,taus,.after=tau) %>% slice(1:5)
t <- tmpdf5w$rowid
tmpdf5s <- tmpdfs[t,]
}
...
return(list(tmpdf1,tmpdf5w,tmpdf5s))
