...
obsresw <- (observed_residuals(df = winter_lap,given = i,v = v) %>% apply(c(2),FUN=row_number))/(nrow(winter_lap)*(1-v)+1)

obsress <- (observed_residuals(df = summer_lap,given = i,v = v) %>% apply(c(2),FUN=row_number))/(nrow(summer_lap)*(1-v)+1)
