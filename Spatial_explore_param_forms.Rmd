...
```{r}
sims_orig <- xyUK20 %>% dplyr::select(!c(lon,lat,x,y)) %>% t() %>% as.data.frame()
# ordered alphabetically so Y1 Birmingham, Y2 Glasgow and Y3 is London
colnames(sims_orig) <- paste0("Y",1:ncol(sims_orig))
# transform to Laplace margins
sims <- as.data.frame((sims_orig %>% apply(c(2),FUN=row_number))/(nrow(sims_orig)+1)) %>% apply(c(1,2),FUN=unif_laplace_pit) %>% as.data.frame()
v <- 0.9 # set threshold

est_all20 <- data.frame("lik" = numeric(), "lika"= numeric(),"likb"= numeric(),"lik2"= numeric(),
                          "a" = numeric(), "b" = numeric(),
                          "mu" = numeric(),"mu_agg" = numeric(),
                          "sig" = numeric(),"sig_agg" = numeric(),"sigl" = numeric(),"sigu" = numeric(),
                          "delta" = numeric(),"deltal" = numeric(), "deltau" = numeric(),
...
for (i in 1:ncol(df_sites)) {
  ...
  for (j in 1:length(dayshift)) {
    ...
    est_all20 <- rbind(est_all20,par_est(df=sims_tau,v=v,given=cond_site,margin = "AGG", method="sequential2",keef_constraints = c(1,2)) %>% add_row(.before=cond_site) %>%  mutate(cond_site=names(df_sites[i]),tau=as.character(dayshift[j])))
  }
}

save(est_all20,file="data_processed/est_all20.RData")

# plot everything
est_all20 <- est_all20 %>% mutate(tau=factor(as.character(tau),levels = as.character(-3:3)))
# save(est_all,est_all_diag,tmp025,tmp075,tmpdiff,file="est_all.RData")

#load("est_all20.RData")
condmodel_params <- c("a","b","mu","sig","muagg","sigl","sigu","sigdiff","deltal","deltau","deltadiff")
# condmodel_params <- c("a","b","mu","sig")
for (i in 1:ncol(df_sites)) {
 tau_site_map <- map_param(tmp_est=est_all20 %>% filter(cond_site==names(df_sites)[i]),method="AGG",facet_var = "tau",title_map = names(df_sites)[i],grid_uk = xyUK20_sf)
   for (j in 1:length(condmodel_params)) {
     tmap_save(tau_site_map[[j]],filename=paste0("../Documents/obs20km_nonstationary/tau_",names(df_sites[i]),"_map_",j,"_",condmodel_params[j],".png"),width=15,height=4.5)
   }
}

# add also maps of alpha and beta


```

Run with new margins.

```{r}
est_all20GG <- data.frame("lik" = numeric(), "lika"= numeric(),"likb"= numeric(),"lik2"= numeric(),
                          "a" = numeric(), "b" = numeric(),
                          "mu" = numeric(),"mu_agg" = numeric(),
                          "sig" = numeric(),"sig_agg" = numeric(),"sigl" = numeric(),"sigu" = numeric(),
                          "delta" = numeric(),"deltal" = numeric(), "deltau" = numeric(),
...
for (i in 1:ncol(df_sites)) {
  ...
  for (j in 1:length(dayshift)) {
    sims_tau <- shift_time(sims=sims,cond_site=cond_site,tau=dayshift[j],Ndays_season = 92)
    est_all20GG <- rbind(est_all20GG,par_est(df=sims_tau,v=v,given=cond_site,margin = "AGG", method="sequentialGG",keef_constraints = c(1,2)) %>% add_row(.before=cond_site) %>%  mutate(cond_site=names(df_sites[i]),tau=as.character(dayshift[j])))
  }
}
```

```{r}
save(est_all20GG,file="data_processed/est_all20GG.RData")

# plot everything
est_all20GG <- est_all20GG %>% mutate(tau=factor(as.character(tau),levels = as.character(-3:3)))
# save(est_all,est_all_diag,tmp025,tmp075,tmpdiff,file="est_all.RData")

#load("data_processed/est_all20GG.RData")
tmp1 <- est_join_spatial(tmp_est=est_all20GG,grid_uk = xyUK20_sf)
tm_shape(tmp1 %>% filter(cond_site=="Birmingham")) + tm_dots(col="delta")
condmodel_params <- c("a","b","mu","sig","muagg","sigl","sigu","sigdiff","deltal","deltau","deltadiff")
# condmodel_params <- c("a","b","mu","sig")
for (i in 1:ncol(df_sites)) {
 tau_site_map <- map_param(tmp_est=est_all20GG %>% filter(cond_site==names(df_sites)[i]),method="AGG",facet_var = "tau",title_map = names(df_sites)[i],grid_uk = xyUK20_sf)
   for (j in 1:length(condmodel_params)) {
     tmap_save(tau_site_map[[j]],filename=paste0("../Documents/obs20km_nonstationary/GG_tau_",names(df_sites[i]),"_map_",j,"_",condmodel_params[j],".png"),width=15,height=4.5)
   }
}

# explore discrepancy
names(df_sites)
unique(est_all20GG$cond_site)
# check AGG estimation
sims10 <- sims[,1:10]
pe10 <- par_est(df=sims10,v=v,given=5,margin = "AGG", method="sequentialGG",keef_constraints = c(1,2))
pe10
# compare parameter estimates with AGG fitted to the observed residuals
obsr <- observed_residuals(df=sims10,given=5,v = 0.9,a=pe10$a,b=pe10$b)
...
petest <- par_est(df=sims,v=v,given=cond_site,margin = "AGG", method="sequentialGG",keef_constraints = c(1,2))
# estimate residual margins
# compare parameter estimates with AGG fitted to the observed residuals
obsr <- observed_residuals(df=sims,given=cond_site,v = 0.9,a=petest$a,b=petest$b)
# fit AGG for each column
dum1 <- data.frame("mu_agg"=numeric(),"sigl"=numeric(),"sigu" = numeric(),"deltal"=numeric(),"deltau" = numeric())
opt2 <- list()
for (i in 1:ncol(obsr)) {
...
# plot to compare
plot(petest$mu_agg,dum1$mu_agg)
# join together and plot (hopefully correct estimates)
i <- 1
pet <- cbind(petest[,c(1:7,9,16,17)],dum1) %>% add_row(.before=cond_site) %>%  mutate(cond_site=names(df_sites[i]),tau=as.character(0))

# plot parameters
i <- 1
petsf <- est_join_spatial(tmp_est=pet,grid_uk=xyUK20_sf)
map_param(tmp_est=petsf %>% filter(cond_site==names(df_sites)[i]),method="AGG",facet_var = "cond_site",title_map = names(df_sites)[i],grid_uk = xyUK20_sf)
```
...
x0999 <- unif_laplace_pit(1-1/(nrow(sims)/40*50))
#x0999 <- unif_laplace_pit(0.999)
q025 <- q050 <- q075 <- numeric()
for (i in 1:nrow(tmp)) {
    # filter value of a and b
  a <- as.numeric(tmp[i,5])
  b <- as.numeric(tmp[i,6])
  if (is.na(a)) {q025[i] <- q050[i] <- q075[i] <- NA} else {
  # find residual variable and cond.variable
  cond_site. <- find_site_index(as.numeric(df_sites[,tmp$cond_site[i]]),grid_uk=xyUK20_sf)
  res_site <- tmp$res[i]
  # calculate Y1 and Yj
  sims_tau <- shift_time(sims=sims,cond_site=cond_site.,tau=as.numeric(as.character(tmp$tau[i])),Ndays_season = nrow(sims)/40) %>% dplyr::select(all_of(c(cond_site.,res_site)))
  names(sims_tau) <- c("Y1","Y2")
  Y1_Y2 <- sims_tau %>% filter(sims_tau[,1]>quantile(sims_tau[,1],v))
  Y1 <- as.numeric(Y1_Y2[,1])
  Y2 <- as.numeric(Y1_Y2[,2])
...
library(ismev)
mu <- sig <- xi <- c()
for (j in 1:ncol(sims_orig)) {
 gpdpar <- ismev::gpd.fit(as.numeric(unlist(sims_orig[,j])),threshold = quantile(as.numeric(unlist(sims_orig[,j])),0.9))$mle
 mu[j] <- quantile(as.numeric(unlist(sims_orig[,j])),0.9)
  sig[j] <- gpdpar[1]
   xi[j] <- gpdpar[2]
}
ysim <- xunif <-  c()
...
for (i in 1:length(q050)) {
 ...
 xunif[i] <-  pfrechet(laplace_frechet_pit(q050[i]))
   res_site <- tmp$res[i]
 X <- as.numeric(unlist(sims_orig[,res_site])) 
...
tm_shape(tmpsf) + tm_dots(col="ysim") + tm_facets(by="cond_site")
 misscol <- "aquamarine"
tm_shape(tmpsf) + tm_dots(col="ysim",style="cont",size=0.3,colorNA=misscol,title=TeX("Simulated temperature"), textNA = "Conditioning site") + tm_facets(by="cond_site",nrow = 1) +  tm_layout(legend.outside.size=0.3,asp=0.5,legend.text.size = 1,legend.title.size=1.5, title="") 

# what are these extreme temperatures?
i <- 3
cond_site <- find_site_index(as.numeric(df_sites[,i]),grid_uk = xyUK20_sf)
qgpd(p=(xunif-0.9)/(1-0.9),loc = mu[cond_site],scale = sig[cond_site],shape = xi[cond_site])


```

## What would be different return levels for London?
...
xq <- unif_laplace_pit(1-1/(nrow(sims)/40*qsim[j]))
for (i in 1:nrow(tmp)) {
    # filter value of a and b
  a <- as.numeric(tmp[i,5])
  b <- as.numeric(tmp[i,6])
  ...
  # find residual variable and cond.variable
  cond_site. <- find_site_index(as.numeric(df_sites[,3]),grid_uk=xyUK20_sf)
  res_site <- tmp$res[i]
  # calculate Y1 and Yj
  sims_tau <- shift_time(sims=sims,cond_site=cond_site.,tau=as.numeric(as.character(tmp$tau[i])),Ndays_season = nrow(sims)/40) %>% dplyr::select(all_of(c(cond_site.,res_site)))
  names(sims_tau) <- c("Y1","Y2")
...
# add as a variable
tmp1 <- rbind(tmp,tmp,tmp) %>% mutate(rl=factor(rep(qsim,each=nrow(tmp)))) %>% mutate(q050=q050)
tmpsf <- est_join_spatial(tmp_est=tmp1,grid_uk=xyUK20_sf)
tm_shape(tmpsf) + tm_dots(col="q050") + tm_facets(by="rl")
tm1 <- tm_shape(tmpsf) + tm_dots(col="q050",style="cont",size=0.3,colorNA=misscol,title=TeX("Simulated temperature"), textNA = "Return level") + tm_facets(by="rl",nrow = 1) +  tm_layout(legend.outside.size=0.3,asp=0.5,legend.text.size = 1,legend.title.size=1.5, title="") 
tmap_save(tm1,filename="../Documents/rl_LOndon.png")
```
