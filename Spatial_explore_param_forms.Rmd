...
tmp_map <- cbind(Birmingham_tau[[1]],data.frame("tau"=dayshift[1]))
for (i in 2:7) {
 tmp_map <- rbind(tmp_map,cbind(Birmingham_tau[[i]],data.frame("tau"=dayshift[i])))
}
tmp_map %>% mutate(tau=factor(as.character(tau)))

map_param(tmp_est=tmp_map,method="AGG")
```
