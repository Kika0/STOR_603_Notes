...
# plot delta
petsf <- est_join_spatial(tmp_est=petest,grid_uk=xyUK20_sf)
map_param(tmp_est=petsf %>% filter(cond_site==names(df_sites)[i]),method="AGG",facet_var = "cond_site",title_map = names(df_sites)[i],grid_uk = xyUK20_sf)
...
tm_shape(tmpsf) + tm_dots(col="a") + tm_facets(by="cond_site")
 misscol <- "aquamarine"
...
# plot alpha for 3 sites

i <- 2
cond_site <- find_site_index(as.numeric(df_sites[,i]),grid_uk = xyUK20_sf)
qgpd(p=((1-1/(nrow(sims)/40*qsim[3]))-0.9)/(1-0.9),loc = mu[cond_site],scale = sig[cond_site],shape = xi[cond_site])
