...
for (i in 1:nrow(tmp)) {
  ...
  res_site <- tmp$res[i]
  # calculate Y1 and Yj
  sims_tau <- shift_time(sims=sims,cond_site=cond_site.,tau=as.numeric(est_all$tau[i])) %>% select(all_of(c(cond_site.,res_site))) 
  names(sims_tau) <- c("Y1","Y2")
...
map_param(tmp_est=tmp %>% filter(cond_site==cond_sites[1]),method = "q", facet_var = c("q","tau"),title_map = cond_sites[1])
...
tmap_save(map_param(tmp_est=tmp %>% filter(cond_site==cond_sites[i]),method = "q", facet_var = c("q","tau"),title_map = cond_sites[i]),filename=paste0("../Documents/est_all_keef/q_tau_",cond_sites[i],".png"),width=12,height=6.5)
}
tmp <- tmp %>% mutate(rl=c(rl025,rl075))
for (i in 1:length(cond_sites)) {
tmap_save(map_param(tmp_est=tmp %>% filter(cond_site==cond_sites[i]),method = "q", facet_var = c("rl","tau"),title_map = cond_sites[i]),filename=paste0("../Documents/est_all_keef/rl_tau_",cond_sites[i],".png"),width=12,height=6.5)
}
