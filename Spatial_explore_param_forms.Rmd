...
```{r}
# test function
spatial_par_est(data_Lap = sims,cond_sites = df_sites %>% dplyr::select(all_of(1)),dayshift = c(-3:3))
...
tm_shape(est_all_sf) + tm_dots(col="lik...1",palette="RdBu") + tm_facets(by="tau",nrow=1)
ggplot(est_all_sf) + geom_point(aes(x=deltal,y=deltau))

# estimate with old data to test
sims1 <- as.data.frame((sims_orig %>% apply(c(2),FUN=row_number))/(nrow(sims_orig)+1)) %>% apply(c(1,2),FUN=unif_laplace_pit) %>% as.data.frame()
library(profvis)
profvis(spatial_par_est(data_Lap = sims1,cond_sites = df_sites %>% dplyr::select(all_of(1)),dayshift = c(0),Ndays_season = 92))
```
