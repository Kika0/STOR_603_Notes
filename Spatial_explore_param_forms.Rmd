...
for (i in 1:ncol(df_sites)) {
    ...
    est_all_nokeef <- rbind(est_all,par_est(df=sims,v=v,given=cond_site,margin = "AGGsigdelta", method="sequential2",keef_constraints = c(0)) %>% add_row(.before=cond_site) %>%  mutate(cond_site=names(df_sites[i]),tau=as.character(0)))
}
...
tmp <- rbind(tmp_est1 %>% add_row(.before=find_site_index(as.numeric(df_sites[,1]))) %>% mutate("cond_site"=names(df_sites)[1]),tmp_est2 %>% add_row(.before=find_site_index(as.numeric(df_sites[,2]))) %>% mutate("cond_site"=names(df_sites)[2]),tmp_est3 %>% add_row(.before=find_site_index(as.numeric(df_sites[,3]))) %>% mutate("cond_site"=names(df_sites)[3])) %>% mutate(cond_site=factor(cond_site))
...
index1 <- find_site_index(as.numeric(df_sites[,4]))
index2 <- find_site_index(as.numeric(df_sites[,5]))
index3 <- find_site_index(as.numeric(df_sites[,6]))
                          
tmp <- rbind(tmp_est4 %>%  add_row(.before=index1) %>% mutate("cond_site"=names(df_sites)[4]),tmp_est5 %>% add_row(.before=index2) %>% mutate("cond_site"=names(df_sites)[5]),tmp_est6 %>% add_row(.before=index3) %>% mutate("cond_site"=names(df_sites)[6])) %>% mutate(cond_site=factor(cond_site))
...
Birmingham_tau <- list()
for (i in 1:7) {
sims_tau <- shift_time(sims=sims,cond_site=cond_site,tau=dayshift[i]) 
Birmingham_tau[[i]] <- par_est(df=sims_tau,v=v,given=cond_site,margin = "AGGsigdelta", method="sequential2")
}
```

```{r}
# plot next to each other
tmp_map <- cbind(Birmingham_tau[[1]] %>%  add_row(.before=cond_site),data.frame("tau"=dayshift[1]))
for (i in 2:7) {
 tmp_map <- rbind(tmp_map,cbind(Birmingham_tau[[i]] %>%  add_row(.before=cond_site),data.frame("tau"=dayshift[i])))
}
tmp_map <- tmp_map %>% mutate(tau=factor(as.character(tau),levels = as.character(-3:3)))
head(tmp_map)
tau_Birm_map <- map_param(tmp_est=tmp_map,method="AGG",facet_var = "tau",title_map = "Birmingham")
tau_Birm_map[[1]]
tmap_save(tau_Birm_map[[1]],filename="tau_Birm_mapa.png",width=15,height=4.5)
condmodel_params <- c("a","b","mu","sig","muagg","sigl","sigu","sigdiff","deltal","deltau","deltadiff")
for (i in 1:length(condmodel_params)) {
 tmap_save(tau_Birm_map[[i]],filename=paste0("tau_Birm_map_",i,"_",condmodel_params[i],".png"),width=15,height=4.5)
}

est_all <- est_all %>% mutate(tau=factor(as.character(tau),levels = as.character(-3:3)))
 map_param(tmp_est=est_all,method="AGG",facet_var = "tau",title_map = "Birmingham")
...
Map and save for all of the sites.
...
tmp <- est_all %>% filter(cond_site %in% cond_sites, tau %in% c("0"))
q025 <- q075 <- numeric()
for (i in 1:nrow(tmp)) {
  ...
  res_site <- tmp$res[i]
  # calculate Y1 and Yj
  Y1_Y2 <- sims[,c(cond_site,res_site)] %>% filter(sims[,cond_site]>quantile(sims[,cond_site],v))
  Y1 <- as.numeric(Y1_Y2[,1]) 
  Y2 <- as.numeric(Y1_Y2[,2])
  # calculate observed residuals
  z <- (Y2-a*Y1)/(Y1^b)
  # calculate q025 and q075
  q025[i] <- a*Y1+Y1^b*quantile(z,0.25)
  q075[i] <- a*Y1+Y1^b*quantile(z,0.75)
  }
}

# join and map
tmp <- tmp %>% mutate(q025,q075) %>%  pivot_longer(,cols=c(q025,q075),names_to = "q") %>% arrange(q) %>% mutate(q=factor(q)) 
tmap_save(map_param(tmp_est=tmp %>% filter(cond_site==cond_sites[1]),method = "q", facet_var = "q",title_map = cond_sites[1]),filename=paste0("../Documents/est_all_keef/q_",cond_sites[1],".png"),width=8,height=6)
tmap_save(map_param(tmp_est=tmp %>% filter(cond_site==cond_sites[2]),method = "q", facet_var = "q",title_map = cond_sites[2]),filename=paste0("../Documents/est_all_keef/q_",cond_sites[2],".png"),width=8,height=6)
tmap_save(map_param(tmp_est=tmp %>% filter(cond_site==cond_sites[3]),method = "q", facet_var = "q",title_map = cond_sites[3]),filename=paste0("../Documents/est_all_keef/q_",cond_sites[3],".png"),width=8,height=6)
