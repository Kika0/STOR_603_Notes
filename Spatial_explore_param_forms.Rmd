...
for (i in 1:length(sites_index_diagonal)) {
  ...
  for (j in 1:length(dayshift)) {
    sims_tau <- shift_time(sims=sims,cond_site=cond_site,tau=dayshift[j])
    est_all_diag <- rbind(est_all_diag,par_est(df=sims_tau,v=v,given=cond_site,margin = "AGG", method="sequential2",keef_constraints = c(1,2)) %>% add_row(.before=cond_site) %>%  mutate(cond_site=sites_index_diagonal[i],tau=as.character(dayshift[j])))
  }
}
```
