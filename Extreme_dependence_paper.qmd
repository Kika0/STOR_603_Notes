...
Rho <- c()
edge <- 10^(-6)
u <- c(seq(0+edge,0.95,0.01),seq(0.95,1-edge,length.out=100))
rho_vec <- seq(-0.9,0.9,0.1)
for (i in 1:length(rho_vec)) {
rho <- rho_vec[i]
# calculate the copula using VGAM package
...
p1 <- ggplot(df) + geom_line(aes(x=U,y=Chi,fill=Rho))+ xlab(TeX("$u$")) + ylab(TeX("$\\chi(u)$")) +  guides(colour = guide_legend(reverse=T)) + labs(fill=TeX("$\rho$"))
p1
```
