...
lims <- c(min(AGGdelta,AGGsig,AGGsigdelta),max(AGGdelta,AGGsig,AGGsigdelta))
...
Now, plot likelihood ration test values instead with $\chi$

```{r}
p1 <- ggplot(data.frame(x=factor(res,levels=c("Z2","Z3","Z4","Z5")),y=AGGsig-AGGdelta),aes(x=x,y=y))+
  geom_boxplot() + xlab("Observed residuals") +
  ylab("Different scale vs shape")
  #ylab(TeX("$l(\\mu,\\sigma_l,\\sigma_u,\\delta_l,\\delta_u)$")) + ylim(lims)
p1
```
