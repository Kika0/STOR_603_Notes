...
`margin="Normal"` means Normal distribution is used for the residual margin as a temporary working assumption.
...
`observed_residuals()` function is used to calculate the observed residuals using the data, threshold, index of conditioning variables and estimates of $\alpha$ and $\beta$ values.

`res_margin_par_est()` function is used to estimate AGG parameters for an observed residual margin (estimates are columns `mu_agg`, `sig_l`, `sig_u`, `delta_l` and `delta_u`).

`vinecop()` function from `rvinecopulib` package is used to fit a vine copula dependence structure to the observe dresiduals transformed onto uniform margins. 
