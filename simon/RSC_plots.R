...
#Birm_index <- find_site_index(site=Cambridge,grid_uk = xyUK20_sf)
London_index <- find_site_index(site=London,grid_uk = xyUK20_sf)
Cambridge <- c(0.1191,52.205)
Cambridge_index <- find_site_index(site=Camb,grid_uk = xyUK20_sf)
Lancaster_index <- find_site_index(site=Lancaster,grid_uk = xyUK20_sf)
tmp <- data_obs_stationary_Lap[,c(Cambridge_index,London_index,Lancaster_index)]
#names(tmp) <- names(df_sites)[c(1,3,5)]
names(tmp) <- c("Cambridge","London","Lancaster")
q <- 0.9
vL <- quantile(tmp[,1],q)
tf <- tmp$tf <- (tmp$London>vL)
limx <-limy <-  c(-10,10)
p1 <- ggplot(tmp) + geom_point(aes(x=London,y=Cambridge),size=0.5) + scale_color_manual(values = c("black", "#009ADA")) + theme(legend.position="none") + coord_fixed() + xlim(limx) + ylim(limy) 
...
p1n <- ggplot(tmp) + geom_point(aes(x=London,y=Cambridge,col=tf),size=0.5) + scale_color_manual(values = c("black", "#009ADA")) + theme(legend.position="none") + coord_fixed() + xlim(limx) + ylim(limy)  + geom_vline(xintercept=vL,color="#009ADA",linetype="dashed")
...
ggsave(p,filename=paste0(folder_name,"condmodel_illustration1.png"),width=8,height=4)
pn <- grid.arrange(p1n,p2n,ncol=2)
ggsave(pn,filename=paste0(folder_name,"condmodel_illustrationvL1.png"),width=8,height = 4)

# 2. plot of London/Cambridge and London/Lancaster on original margins -------
tmp <- data_obs_all[,c(Cambridge_index,London_index,Lancaster_index)]
names(tmp) <- c("Cambridge","London","Lancaster")
size_point <- 0.3
tmp$tf <- tf
limx <-limy <-  c(7,40)
p1 <- ggplot(tmp) + geom_point(aes(x=London,y=Cambridge),size=size_point) + coord_fixed() + xlim(limx) + ylim(limy) 
p2 <- ggplot(tmp) + geom_point(aes(x=London,y=Lancaster),size=size_point) + coord_fixed() + xlim(limx) + ylim(limy) 
p <- grid.arrange(p1,p2,ncol=2)
ggsave(p,filename= paste0(folder_name,"London_Cambridge_Lancaster_original.png"),width=8,height=4)
tmp <- tmp %>% mutate(tf=factor(tf,levels=c(TRUE,FALSE)))
p1 <- ggplot(tmp) + geom_point(aes(x=London,y=Cambridge,col=tf),size=size_point) + scale_color_manual(values = c("black", "#009ADA")) + theme(legend.position="none") + coord_fixed() + xlim(limx) + ylim(limy) 
...
p <- grid.arrange(p1,p2,ncol=2)
ggsave(p,filename= paste0(folder_name,"London_Cambridge_Lancaster_original_above.png"),width=8,height=4)
...
aest <- my_df %>% filter(cond_site=="London") %>% pull(alpha)
#best <- discard(est_all_sf %>% filter(cond_site=="London") %>% pull(b),is.na)
best <- my_df %>% filter(cond_site=="London") %>% pull(b_re_est)
...
pe <- my_df%>% filter(cond_site=="London") %>% select(mu,sigl,sigu,deltal,deltau)
names(pe) <- c("mu","sigl","sigu","deltal","deltau")
# get fields
# reconstruct the fields
y_sim <- apply(random10N[-London_index,],MARGIN=c(2),FUN=function(xk){xL*aest+xL^best*xk})
...
x1sub <- x_sim1[,c(3,1,10)]
x2sub <- x_sim2[,c(3,1,10)]
...
t2 <- tm_shape(tmpsf %>% mutate("name"=factor(name, levels=unique(tmpsf$name))) %>% dplyr::filter(name=="random1")) + tm_dots(fill="value",size=0.5,fill.scale = tm_scale_continuous(values="-brewer.rd_bu",limits=lims),fill.legend = tm_legend(title = "Temperature")) + tm_layout(legend.show=FALSE,legend.height = leg_height,frame=FALSE) 
...
t5 <- tm_shape(tmpsf %>% mutate("name"=factor(name, levels=unique(tmpsf$name)))%>% filter(name=="random1")) + tm_dots(fill="value",size=0.5,fill.scale = tm_scale_continuous(values="-brewer.rd_bu",limits=lims),fill.legend = tm_legend(title = "Temperature")) + tm_layout(legend.show=FALSE,legend.height = leg_height,frame=FALSE) 
...
tmp <- Z1[,c(Cambridge_index,Lancaster_index,Inverness_index)] 
names(tmp) <- c("Cambridge","Lancaster","Inverness")
...
# get the parameters
pe3 <- (pe %>% add_row(.before=London_index))[c(Cambridge_index,Lancaster_index,Inverness_index),]
