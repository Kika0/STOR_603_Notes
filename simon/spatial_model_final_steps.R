...
aest <- est_all_sf %>% filter(cond_site=="London") %>% pull(a)
best <- est_all_sf %>% filter(cond_site=="London") %>% pull(b)
summary(aest)
summary(best)
...
NLL_AGG_wrapper <- function(data_Lap=data_mod_Lap,i,pe_res=pe,cond_index,v=0.9,a=aest,b=best) {
  pe_i <- as.numeric(unlist(pe_res[i,]))
  data_Lapv <- data_Lap %>% filter(data_Lap[,cond_index]>quantile(data_Lap[,cond_index],v))
  x1 <- as.numeric(unlist(data_Lapv[,London_index]))
